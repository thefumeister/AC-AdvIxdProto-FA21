...
void setup() {
  ...
  //serial setup, analog ports: A1, A2  digital port: D13
...
void draw() {
  ...
  if(crosshair.mode == 2) { 
    ...
    if(abs(crosshair.position.x - miner.position.x) <= 15 && abs(crosshair.position.y - miner.position.y) <= 15) {
      crosshair.mode = 3;
    }
  }
  ...
  // launch probe on button press, consume resources each launch
...
void serialEvent(Serial conn) {
  ...
  if (values.length == 3 && !Float.isNaN(values[0]) && !Float.isNaN(values[1])) {
    // joystick x and y potentiometer inputs, compensation for minor fluctuations
