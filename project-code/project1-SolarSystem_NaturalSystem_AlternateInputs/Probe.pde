class Probe {
  ...
  void move(float _pointer) {
    
    if (pointer_mode == false) {
      //pointer_x = map(float(serialValue), 0, 4095, 0, width);
      pointer_x = _pointer;
    }
    if (pointer_mode == true) {
      //pointer_y = map(float(serialValue), 0, 4095, 0, height);
      pointer_y = _pointer;
    }
    
    //println(float(serialValue));
     ...
     try {
        if (abs(targetAsteroid.position.x - position.x) <= 100 && abs(targetAsteroid.position.y - position.y) <= 100) {
          if (asteroids.size() >= 1 && (full == false)) { 
            targetAsteroid.minerals -= 1;
            targetAsteroid.radiusL -= 25;
            targetAsteroid.radiusW -= 25;
            full = true;
